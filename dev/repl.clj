...
;; ----------------------------------------------------------------------------------------
;; If you're new to REPL-driven development, Biff makes it easy to get started:
;; whenever
;; you save a file, your changes will be evaluated. Biff is structured so that
;; in most
;; cases, that's all you'll need to do for your changes to take effect. (See
;; main/refresh
;; below for more details.)
;;
;; The `clj -M:dev dev` command also starts an nREPL server on port 7888, so if
;; you're
;; already familiar with REPL-driven development, you can connect to that with
;; your editor.
;;
;; If you're used to jacking in with your editor first and then starting your
;; app via the
;; REPL, you will need to instead connect your editor to the nREPL server that
;; `clj -M:dev
;; dev` starts. e.g. if you use emacs, instead of running `cider-jack-in`, you
;; would run
...
(defn get-context [] (biff/merge-context @main/system))
...
(defn check-config
  ...
  (let [prod-config (biff/use-aero-config {:biff.config/profile "prod"})
        ...
        secret-keys [:biff.middleware/cookie-secret :biff/jwt-secret
                     :mailersend/api-key :recaptcha/secret-key]
        ; ...
        get-secrets (fn [{:keys [biff/secret], :as config}]
                      (into {} (map (fn [k] [k (secret k)])) secret-keys))]
