...
(defn bar-form
  [{:keys [value]}]
  (biff/form
    {:hx-post "/app/set-bar", :hx-swap "outerHTML"}
    [:label.block {:for "bar"} "Bar: " [:span.font-mono (pr-str value)]]
    [:.h-1]
    [:.flex [:input.w-full#bar {:type "text", :name "bar", :value value}]
...
(defn chat
  ...
  (let [messages (q db
    ...
    [:div {:hx-ext "ws", :ws-connect "/app/chat"}
     [:form.mb-0 {:ws-send true, :_ "on submit set value of #message to ''"}
      [:label.block {:for "message"} "Write a message"] [:.h-1]
      [:textarea.w-full#message {:name "text"}] [:.h-1]
      [:.text-sm.text-gray-600
       "Sign in with an incognito window to have a conversation with yourself."]
      [:.h-2] [:div [:button.btn {:type "submit"} "Send message"]]] [:.h-6]
     [:div#message-header
...
(defn app
  ...
  (let [{:user/keys [email foo bar]} (xt/entity db (:uid session))]
    (ui/page
      ...
      (biff/form
        {:action "/app/set-foo"}
        [:label.block {:for "foo"} "Foo: " [:span.font-mono (pr-str foo)]]
        [:.h-1]
        [:.flex [:input.w-full#foo {:type "text", :name "foo", :value foo}]
         [:.w-3] [:button.btn {:type "submit"} "Update"]]
        [:.h-1]
        [:.text-sm.text-gray-600
...
(def module
  {:static {"/about/" about-page},
   :routes ["/app" {:middleware [mid/wrap-signed-in]} ["" {:get app}]
            ["/set-foo" {:post set-foo}] ["/set-bar" {:post set-bar}]
            ["/chat" {:get ws-handler}]],
