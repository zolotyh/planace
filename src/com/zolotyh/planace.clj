...
(def routes
  [["" {:middleware [mid/wrap-site-defaults]} (keep :routes modules)]
   ["" {:middleware [mid/wrap-api-defaults]} (keep :api-routes modules)]])
...
(defn generate-assets!
  ...
  (biff/delete-old-files {:dir "target/resources/public", :exts [".html"]}))
...
(def malli-opts
  {:registry (malr/composite-registry malc/default-registry
...
(def components
  [biff/use-aero-config biff/use-xtdb biff/use-queues biff/use-xtdb-tx-listener
   biff/use-htmx-refresh biff/use-jetty biff/use-chime biff/use-beholder])
...
(defn -main
  []
  (let [{:keys [biff.nrepl/args]} (start)] (apply nrepl-cmd/-main args)))
